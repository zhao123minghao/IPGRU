...
def vae_main():
    ...
    weight_path = f'/home/yangtianyu/ljs_data/AutoImpute/checkpoint/{Dataset_name}/weight_gru'+'.ckpt'
    ...
    history=model.fit(input_train_single,train_cells,validation_data=(input_test_single,test_cells),epochs=50,batch_size=32,validation_freq=1,callbacks=[cp_callback,EarlyStopping(monitor='val_loss', patience=3, verbose=1, mode='min')])
    # print(history.history)
    val_loss=history.history['val_loss']
    loss=history.history['loss']

    save_dir = f'/home/yangtianyu/ljs_data/AutoImpute/loss/val_nz_loss/{Dataset_name}/'
    os.makedirs(save_dir, exist_ok=True)  # 如果目录不存在，则自动创建
    np.savetxt(f'{save_dir}/val_nz_loss.txt', val_loss, fmt='%s')
    # 创建目录
    save_dir_train = f'/home/yangtianyu/ljs_data/AutoImpute/loss/train_nz_loss/{Dataset_name}/'
    os.makedirs(save_dir_train, exist_ok=True)  # 如果目录不存在，则自动创建
    # 保存文件
    np.savetxt(f'{save_dir_train}/train_nz_loss.txt', loss, fmt='%s')
    # for j in val_loss:
    #     min=10
    #     if min>j:
    #         min=j
    ...
    save_dir_imputed = f'/home/yangtianyu/ljs_data/AutoImpute/imputed/{Dataset_name}/'
    os.makedirs(save_dir_imputed, exist_ok=True)  # 自动创建目录

    # 确保保存 masked_input 结果的目录存在
    save_dir_masked_input = f'/home/yangtianyu/ljs_data/AutoImpute/masked_input/{Dataset_name}/'
    os.makedirs(save_dir_masked_input, exist_ok=True)  # 自动创建目录

    scimpute.save_hd5(Imputed_df,f'/home/yangtianyu/ljs_data/AutoImpute/imputed/{Dataset_name}/imputation.hd5')
    scimpute.save_hd5(Y_df,f'/home/yangtianyu/ljs_data/AutoImpute/imputed/{Dataset_name}/ground_truth.hd5')
    scimpute.save_hd5(masked_input,f'/home/yangtianyu/ljs_data/AutoImpute/masked_input/{Dataset_name}/masked_input.hd5')
    # 最优模型保存
    # vae.summary()
